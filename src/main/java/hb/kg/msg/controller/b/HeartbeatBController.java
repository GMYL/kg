 ...
 * 这个类用于处理heartbeat的信号
 */
@Api(description = "[B]处理heartbeat的信号")
@RestController
@RequestMapping(value = "/${api.version}/b/msg/heartbeat")
public class HeartbeatBController extends BaseController {
    @Autowired
    private SiteMailService siteMailService;
    @Resource
    private StringRedisDao stringRedisDao;

    @ApiOperation(value = "信息监测-[List]", notes = "信息监测-[List]，检查用户是否需要访问数据库，检查用户是否有站内信", produces = "application/json")
    ...
    public ResponseBean checkList(HeartbeatBean heartbeat) {
        ...
        if (!StringUtils.isEmpty(userid)) {
            ...
            // 返回当前在线人数
            rsMap.put("online", stringRedisDao.getUserOnline());
            // 更新redis中用户的状态
