 ...
 */
@Resource
public abstract class BaseMongoDao<T extends BaseIdBean> extends BaseDao<T> {
    @Resource
    ...
    /**
     * 更新一个map对象，等同于覆盖式insert, 注意map中存在id为key键
     */
    public boolean updateOne(Map<String, Object> kvs) {
        if (kvs.get("id") != null) {
            return updateOne(kvs.get("id").toString(), kvs, false);
        }
        return false;
    }
    ...
    public boolean removeAll(Map<String, Object> params) {
        this.preUpdate();
        DeleteResult result = mongoTemplate.remove(new BasicQuery(new Document(params)),
                                                   getClassT());
        this.postUpdate();
        return result.getDeletedCount() > 0;
    }
    ...
    public boolean removeAll(Query query) {
        this.preUpdate();
        DeleteResult result = mongoTemplate.remove(query, getClassT());
        this.postUpdate();
        return result.getDeletedCount() > 0;
    }
