...
public class AuthImageCode extends BaseController {
    ...
    @ApiOperation(value = "获取用户验证码", notes = "获取用户验证码", produces = "application/json")
    ...
    public void getAuthCode(HttpServletRequest request,
        ...
        stringRedisDao.insertOne(sessionCode + ".auth_img_code", strCode, 5 * 60);
    ...
    @ApiOperation(value = "验证用户验证码", notes = "验证用户验证码", produces = "application/json")
    @RequestMapping(value = "/checkAuthCode/{code}")
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public JSONObject checkAuthCode(HttpServletRequest request,
                                    HttpServletResponse response,
                                    @ApiParam(value = "验证码", required = true) @PathVariable String code)
            throws IOException {
