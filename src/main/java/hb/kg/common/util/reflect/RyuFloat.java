...
public final class RyuFloat {
    private static final int[][] POW5_SPLIT = { { 536870912, 0 },
                                                { 671088640, 0 },
                                                { 838860800, 0 },
                                                { 1048576000, 0 },
                                                { 655360000, 0 },
    ...
    private static final int[][] POW5_INV_SPLIT = { { 268435456, 1 },
    ...
    public static int toString(float value,
        ...
        // By default, the correct mantissa starts with a 1, except for denormal
        // numbers.
        int e2;
        int m2;
        if (ieeeExponent == 0) {
        ...
        if (e2 >= 0) {
            ...
            if (q != 0 && ((dp - 1) / 10 <= dm / 10)) {
                ...
                // could use
                // q = X - 1 above, except that would require 33 bits for the result, and we've
                // found that
                // 32-bit arithmetic is faster even on 64-bit machines.
                int e = q - 1;
                int l = 59 + (e == 0 ? 1 : (int) ((e * 23219280L + 10000000L - 1) / 10000000L)) - 1;
                int qx = q - 1, ii = -e2 + q - 1 + l;
                long mulPow5InvDivPow2 = (mv * (long) POW5_INV_SPLIT[qx][0]
                        + ((mv * (long) POW5_INV_SPLIT[qx][1]) >> 31)) >> (ii - 31);
            ...
            if (q != 0 && ((dp - 1) / 10 <= dm / 10)) {
                int e = i + 1;
                j = q - 1
                        - ((e == 0 ? 1 : (int) ((e * 23219280L + 10000000L - 1) / 10000000L)) - 61);
                int ix = i + 1;
                long mulPow5divPow2 = (mv * (long) POW5_SPLIT[ix][0]
                        + ((mv * (long) POW5_SPLIT[ix][1]) >> 31)) >> (j - 31);
        ...
        // Float.toString semantics requires using scientific notation if and only if
        // outside this range.
