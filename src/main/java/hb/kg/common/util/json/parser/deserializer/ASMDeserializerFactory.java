...
import hb.kg.common.util.json.asm.ASMClassLoader;
import hb.kg.common.util.json.asm.ASMType;
import hb.kg.common.util.json.asm.ASMUtils;
import hb.kg.common.util.json.asm.ClassWriter;
import hb.kg.common.util.json.asm.FieldWriter;
import hb.kg.common.util.json.asm.Label;
import hb.kg.common.util.json.asm.MethodVisitor;
import hb.kg.common.util.json.asm.MethodWriter;
import hb.kg.common.util.json.asm.Opcodes;
import hb.kg.common.util.json.parser.DefaultJSONParser;
import hb.kg.common.util.json.parser.DefaultJSONParser.ResolveTask;
...
public class ASMDeserializerFactory implements Opcodes {
    ...
    final static String DefaultJSONParserStr = ASMUtils.type(DefaultJSONParser.class);
    final static String JSONLexerBaseStr = ASMUtils.type(JSONLexerBase.class);
    ...
    public ObjectDeserializer createJavaBeanDeserializer(ParserConfig config,
        ...
        String className = "FastjsonASMDeserializer_" + seed.incrementAndGet() + "_"
                + clazz.getSimpleName();
        ...
        cw.visit(V1_5,
                 ...
                 classNameType,
                 ASMUtils.type(JavaBeanDeserializer.class),
                 null);
        ...
        Constructor<?> constructor = deserClass.getConstructor(ParserConfig.class,
                                                               JavaBeanInfo.class);
        Object instance = constructor.newInstance(config, beanInfo);
        return (ObjectDeserializer) instance;
    }

    private void _setFlag(MethodVisitor mw,
                          Context context,
                          int i) {
    ...
    private void _isFlag(MethodVisitor mw,
                         Context context,
                         int i,
                         Label label) {
    ...
    private void _deserialzeArrayMapping(ClassWriter cw,
                                         Context context) {
        MethodVisitor mw = new MethodWriter(cw,
                                            ACC_PUBLIC,
                                            "deserialzeArrayMapping",
                                            "(L" + DefaultJSONParserStr
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == byte.class //
                ...
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanInt", "(C)I");
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitFieldInsn(GETFIELD, JSONLexerBaseStr, "matchStat", "I");
                mw.visitLdcInsn(JSONLexerBase.VALUE_NULL);
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanLong", "(C)J");
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanBoolean", "(C)Z");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.name + "_asm"));
            } else if (fieldClass == float.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFloat", "(C)F");
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanDouble", "(C)D");
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "getCurrent", "()C");
                mw.visitInsn(DUP);
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(fieldClass)));
                mw.visitVarInsn(ALOAD, 1);
                mw.visitMethodInsn(INVOKEVIRTUAL,
                                   DefaultJSONParserStr,
                                   "getSymbolTable",
                                   "()" + ASMUtils.desc(SymbolTable.class));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL,
                                   JSONLexerBaseStr,
                                   "scanEnum",
                                   "(Ljava/lang/Class;" + ASMUtils.desc(SymbolTable.class)
                                           + "C)Ljava/lang/Enum;");
                ...
                mw.visitJumpInsn(IF_ICMkgT, enumNumErr_);
                _getFieldDeser(context, mw, fieldInfo);
                mw.visitTypeInsn(CHECKCAST, ASMUtils.type(EnumDeserializer.class)); // cast
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanInt", "(C)I");
                mw.visitMethodInsn(INVOKEVIRTUAL,
                                   ASMUtils.type(EnumDeserializer.class),
                                   "valueOf",
                                   "(I)Ljava/lang/Enum;");
                mw.visitJumpInsn(GOTO, enumStore_);
                ...
                mw.visitMethodInsn(INVOKEVIRTUAL,
                                   ASMUtils.type(JavaBeanDeserializer.class),
                                   "scanEnum",
                                   "(L" + JSONLexerBaseStr + ";C)Ljava/lang/Enum;");
                mw.visitLabel(enumStore_);
                mw.visitTypeInsn(CHECKCAST, ASMUtils.type(fieldClass)); // cast
                mw.visitVarInsn(ASTORE, context.var(fieldInfo.name + "_asm"));
            } else if (Collection.class.isAssignableFrom(fieldClass)) {
                Class<?> itemClass = TypeUtils.getCollectionItemClass(fieldType);
                if (itemClass == String.class) {
                    if (fieldClass == List.class || fieldClass == Collections.class
                            || fieldClass == ArrayList.class) {
                        mw.visitTypeInsn(NEW, ASMUtils.type(ArrayList.class));
                        mw.visitInsn(DUP);
                        mw.visitMethodInsn(INVOKESPECIAL,
                                           ASMUtils.type(ArrayList.class),
                                           "<init>",
                                           "()V");
                    } else {
                        mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(fieldClass)));
                        mw.visitMethodInsn(INVOKESTATIC,
                                           ASMUtils.type(TypeUtils.class),
                                           "createCollection",
                    ...
                    mw.visitVarInsn(ALOAD, context.var("lexer"));
                    mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "token", "()I");
                    ...
                    mw.visitVarInsn(ALOAD, 1); // DefaultJSONParserStr
                    mw.visitVarInsn(ILOAD, context.var("token"));
                    mw.visitMethodInsn(INVOKEVIRTUAL,
                                       DefaultJSONParserStr,
                                       "throwException",
                    ...
                    mw.visitVarInsn(ALOAD, context.var("lexer"));
                    mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "next", "()C");
                    mw.visitInsn(POP);
                    mw.visitVarInsn(ALOAD, context.var("lexer"));
                    mw.visitLdcInsn(JSONToken.LBRACKET);
                    mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "setToken", "(I)V");
                    mw.visitJumpInsn(GOTO, quickEnd_);
                    mw.visitLabel(quickElse_);
                    mw.visitVarInsn(ALOAD, context.var("lexer"));
                    mw.visitLdcInsn(JSONToken.LBRACKET);
                    mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "nextToken", "(I)V");
                    ...
                    mw.visitVarInsn(ALOAD, 1);
                    mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(itemClass)));
                    mw.visitVarInsn(ALOAD, 3);
                    mw.visitMethodInsn(INVOKESTATIC,
                                       ASMUtils.type(JavaBeanDeserializer.class),
                                       "parseArray",
                                       "(Ljava/util/Collection;" //
                                               + ASMUtils.desc(ObjectDeserializer.class) //
                                               + "L" + DefaultJSONParserStr + ";" //
                ...
                if (fieldClass == java.util.Date.class) {
                    ...
                    mw.visitTypeInsn(NEW, ASMUtils.type(java.util.Date.class));
                    mw.visitInsn(DUP);
                    mw.visitVarInsn(ALOAD, context.var("lexer"));
                    mw.visitVarInsn(BIPUSH, seperator);
                    mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanLong", "(C)J");
                    mw.visitMethodInsn(INVOKESPECIAL,
                                       ASMUtils.type(java.util.Date.class),
                                       "<init>",
                                       "(J)V");
                    mw.visitVarInsn(ASTORE, context.var(fieldInfo.name + "_asm"));
                ...
                mw.visitMethodInsn(INVOKESPECIAL, //
                                   ASMUtils.type(JavaBeanDeserializer.class), //
                                   "check",
                                   "(" + ASMUtils.desc(JSONLexer.class) + "I)V");
    ...
    private void _deserialze(ClassWriter cw,
                             Context context) {
        ...
        defineVarLexer(context, mw);
        {
            ...
            if ((beanInfo.parserFeatures & Feature.SupportArrayToBean.mask) == 0) {
                ...
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "isEnabled", "(II)Z");
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "isEnabled", "(I)Z");
        mw.visitJumpInsn(IFEQ, super_);
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitLdcInsn(context.clazz.getName());
        mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanType", "(Ljava/lang/String;)I");
        mw.visitLdcInsn(JSONLexerBase.NOT_MATCH);
        mw.visitJumpInsn(IF_ICMPEQ, super_);
        mw.visitVarInsn(ALOAD, 1); // parser
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           DefaultJSONParserStr,
                           "getContext",
                           "()" + ASMUtils.desc(ParseContext.class));
        ...
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitFieldInsn(GETFIELD, JSONLexerBaseStr, "matchStat", "I");
        mw.visitLdcInsn(JSONLexerBase.END);
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == boolean.class) {
                ...
                mw.visitFieldInsn(GETFIELD,
                                  ...
                                  "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFieldBoolean", "([C)Z");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.name + "_asm"));
            } else if (fieldClass == byte.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(ALOAD, 0);
                mw.visitFieldInsn(GETFIELD,
                                  ...
                                  fieldInfo.name + "_asm_prefix__",
                                  "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFieldInt", "([C)I");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.name + "_asm"));
            } else if (fieldClass == Byte.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(ALOAD, 0);
                ...
                mw.visitFieldInsn(GETFIELD,
                                  ...
                                  fieldInfo.name + "_asm_prefix__",
                                  "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFieldLong", "([C)J");
                mw.visitVarInsn(LSTORE, context.var(fieldInfo.name + "_asm", 2));
            } else if (fieldClass == Long.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(ALOAD, 0);
                ...
                mw.visitFieldInsn(GETFIELD,
                                  ...
                                  fieldInfo.name + "_asm_prefix__",
                                  "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFieldFloat", "([C)F");
                mw.visitVarInsn(FSTORE, context.var(fieldInfo.name + "_asm"));
            } else if (fieldClass == Float.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(ALOAD, 0);
                ...
                mw.visitFieldInsn(GETFIELD,
                                  ...
                                  fieldInfo.name + "_asm_prefix__",
                                  "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFieldDouble", "([C)D");
                mw.visitVarInsn(DSTORE, context.var(fieldInfo.name + "_asm", 2));
            } else if (fieldClass == Double.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(ALOAD, 0);
                ...
                mw.visitFieldInsn(GETFIELD,
                                  ...
                                  fieldInfo.name + "_asm_prefix__",
                                  "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFieldIntArray", "([C)[I");
                mw.visitVarInsn(ASTORE, context.var(fieldInfo.name + "_asm"));
            } else if (fieldClass == float[].class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(ALOAD, 0);
                ...
                // mw.visitVarInsn(ALOAD, context.var("lexer"));
                // mw.visitVarInsn(ALOAD, 0);
                // mw.visitFieldInsn(GETFIELD, context.className, fieldInfo.name +
                // "_asm_prefix__", "[C");
                ...
                // mw.visitVarInsn(ALOAD, 1);
                //
                // mw.visitMethodInsn(INVOKEVIRTUAL, DefaultJSONParserStr, "getSymbolTable",
                // "()" +
                // desc(SymbolTable.class));
                //
                // mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "scanFieldSymbol",
                ...
                Class<?> itemClass = TypeUtils.getCollectionItemClass(fieldType);
                if (itemClass == String.class) {
                    mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(fieldClass))); // cast
                    mw.visitMethodInsn(INVOKEVIRTUAL,
                                       JSONLexerBaseStr,
                                       "scanFieldStringArray",
                                       "([CLjava/lang/Class;)" + ASMUtils.desc(Collection.class));
                    mw.visitVarInsn(ASTORE, context.var(fieldInfo.name + "_asm"));
                } else {
        ...
        if (buildMethod != null) {
            mw.visitMethodInsn(INVOKEVIRTUAL,
                               ASMUtils.type(context.getInstClass()),
                               buildMethod.getName(),
                               "()" + ASMUtils.desc(buildMethod.getReturnType()));
        }
        mw.visitInsn(ARETURN);
        ...
        mw.visitTypeInsn(CHECKCAST, ASMUtils.type(context.clazz)); // cast
        mw.visitInsn(ARETURN);
    ...
    private void defineVarLexer(Context context,
                                MethodVisitor mw) {
        mw.visitVarInsn(ALOAD, 1);
        mw.visitFieldInsn(GETFIELD, DefaultJSONParserStr, "lexer", ASMUtils.desc(JSONLexer.class));
        mw.visitTypeInsn(CHECKCAST, JSONLexerBaseStr); // cast
        mw.visitVarInsn(ASTORE, context.var("lexer"));
    }

    private void _createInstance(Context context,
                                 MethodVisitor mw) {
        JavaBeanInfo beanInfo = context.beanInfo;
        Constructor<?> defaultConstructor = beanInfo.defaultConstructor;
        if (Modifier.isPublic(defaultConstructor.getModifiers())) {
            mw.visitTypeInsn(NEW, ASMUtils.type(context.getInstClass()));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL,
                               ASMUtils.type(defaultConstructor.getDeclaringClass()),
                               "<init>",
                               "()V");
            ...
            mw.visitMethodInsn(INVOKESPECIAL,
                               ...
                               "(L" + DefaultJSONParserStr
                                       + ";Ljava/lang/reflect/Type;)Ljava/lang/Object;");
            mw.visitTypeInsn(CHECKCAST, ASMUtils.type(context.getInstClass())); // cast
            mw.visitVarInsn(ASTORE, context.var("instance"));
        }
    }

    private void _batchSet(Context context,
                           MethodVisitor mw) {
    ...
    private void _loadAndSet(Context context,
                             MethodVisitor mw,
                             FieldInfo fieldInfo) {
        Class<?> fieldClass = fieldInfo.fieldClass;
        Type fieldType = fieldInfo.fieldType;
        if (fieldClass == boolean.class) {
            ...
            if (fieldInfo.method != null) {
                mw.visitMethodInsn(INVOKEVIRTUAL,
                                   ASMUtils.type(context.getInstClass()),
                                   fieldInfo.method.getName(),
                                   ASMUtils.desc(fieldInfo.method));
                if (!fieldInfo.method.getReturnType().equals(Void.TYPE)) {
                    mw.visitInsn(POP);
                }
            } else {
                mw.visitFieldInsn(PUTFIELD,
                                  ASMUtils.type(fieldInfo.declaringClass),
                                  fieldInfo.field.getName(),
                                  ASMUtils.desc(fieldInfo.fieldClass));
            }
        } else if (fieldClass == float.class) {
            mw.visitVarInsn(ALOAD, context.var("instance"));
    ...
    private void _set(Context context,
                      MethodVisitor mw,
                      FieldInfo fieldInfo) {
        Method method = fieldInfo.method;
        if (method != null) {
            Class<?> declaringClass = method.getDeclaringClass();
            mw.visitMethodInsn(declaringClass.isInterface() ? INVOKEINTERFACE : INVOKEVIRTUAL,
                               ASMUtils.type(fieldInfo.declaringClass),
                               method.getName(),
                               ASMUtils.desc(method));
            if (!fieldInfo.method.getReturnType().equals(Void.TYPE)) {
                mw.visitInsn(POP);
            }
        } else {
    ...
    private void _setContext(Context context,
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           DefaultJSONParserStr,
                           "setContext",
                           "(" + ASMUtils.desc(ParseContext.class) + ")V");
        ...
        mw.visitVarInsn(ALOAD, context.var("instance"));
        mw.visitFieldInsn(PUTFIELD,
                          ASMUtils.type(ParseContext.class),
                          "object",
                          "Ljava/lang/Object;");
        mw.visitLabel(endIf_);
    }

    private void _deserialize_endCheck(Context context,
                                       MethodVisitor mw,
                                       Label reset_) {
    ...
    private void _deserialze_list_obj(Context context,
                                      MethodVisitor mw,
                                      Label reset_,
                                      FieldInfo fieldInfo,
                                      Class<?> fieldClass,
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "matchField", "([C)Z");
        ...
        mw.visitFieldInsn(GETFIELD,
                          context.className,
                          fieldInfo.name + "_asm_list_item_deser__",
                          ASMUtils.desc(ObjectDeserializer.class));
        mw.visitVarInsn(ALOAD, 1);
        mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(itemType)));
    ...
    private void _quickNextToken(Context context,
                                 MethodVisitor mw,
                                 int token) {
        Label quickElse_ = new Label(), quickEnd_ = new Label();
        mw.visitVarInsn(ALOAD, context.var("lexer"));
    ...
    private void _quickNextTokenComma(Context context,
                                      MethodVisitor mw) {
        Label quickElse_ = new Label(), quickElseIf0_ = new Label(), quickElseIf1_ = new Label(),
                quickElseIf2_ = new Label(), quickEnd_ = new Label();
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "getCurrent", "()C");
        mw.visitInsn(DUP);
        mw.visitVarInsn(ISTORE, context.var("ch"));
        ...
        mw.visitLabel(quickElse_);
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitMethodInsn(INVOKEVIRTUAL, JSONLexerBaseStr, "nextToken", "()V");
        mw.visitLabel(quickEnd_);
    }

    private void _getCollectionFieldItemDeser(Context context,
                                              MethodVisitor mw,
                                              FieldInfo fieldInfo,
                                              Class<?> itemType) {
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           DefaultJSONParserStr,
                           "getConfig",
                           "()" + ASMUtils.desc(ParserConfig.class));
        mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(itemType)));
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           ASMUtils.type(ParserConfig.class),
                           "getDeserializer",
                           "(Ljava/lang/reflect/Type;)" + ASMUtils.desc(ObjectDeserializer.class));
        mw.visitFieldInsn(PUTFIELD,
                          context.className,
                          fieldInfo.name + "_asm_list_item_deser__",
                          ASMUtils.desc(ObjectDeserializer.class));
    ...
    private void _newCollection(MethodVisitor mw,
                                Class<?> fieldClass,
                                int i,
                                boolean set) {
        if (fieldClass.isAssignableFrom(ArrayList.class) && !set) {
            ...
            mw.visitTypeInsn(NEW, ASMUtils.type(LinkedList.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.type(LinkedList.class), "<init>", "()V");
        } else if (fieldClass.isAssignableFrom(HashSet.class)) {
            mw.visitTypeInsn(NEW, ASMUtils.type(HashSet.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.type(HashSet.class), "<init>", "()V");
        } else if (fieldClass.isAssignableFrom(TreeSet.class)) {
            mw.visitTypeInsn(NEW, ASMUtils.type(TreeSet.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.type(TreeSet.class), "<init>", "()V");
        } else if (fieldClass.isAssignableFrom(LinkedHashSet.class)) {
            mw.visitTypeInsn(NEW, ASMUtils.type(LinkedHashSet.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.type(LinkedHashSet.class), "<init>", "()V");
        } else if (set) {
            mw.visitTypeInsn(NEW, ASMUtils.type(HashSet.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.type(HashSet.class), "<init>", "()V");
    ...
    private void _deserialze_obj(Context context,
                                 MethodVisitor mw,
                                 Label reset_,
                                 FieldInfo fieldInfo,
                                 Class<?> fieldClass,
        ...
        _deserObject(context, mw, fieldInfo, fieldClass, i);
        mw.visitVarInsn(ALOAD, 1);
        mw.visitMethodInsn(INVOKEVIRTUAL, DefaultJSONParserStr, "getResolveStatus", "()I");
        mw.visitLdcInsn(DefaultJSONParser.NeedToResolve);
        mw.visitJumpInsn(IF_ICMPNE, _end_if);
        mw.visitVarInsn(ALOAD, 1);
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           DefaultJSONParserStr,
                           "getLastResolveTask",
                           "()" + ASMUtils.desc(ResolveTask.class));
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           ...
                           "()" + ASMUtils.desc(ParseContext.class));
        mw.visitFieldInsn(PUTFIELD,
                          ASMUtils.type(ResolveTask.class),
                          "ownerContext",
                          ASMUtils.desc(ParseContext.class));
        mw.visitVarInsn(ALOAD, context.var("resolveTask"));
        mw.visitVarInsn(ALOAD, 0);
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           ...
                           "(Ljava/lang/String;)" + ASMUtils.desc(FieldDeserializer.class));
        mw.visitFieldInsn(PUTFIELD,
                          ASMUtils.type(ResolveTask.class),
                          "fieldDeserializer",
                          ASMUtils.desc(FieldDeserializer.class));
        mw.visitVarInsn(ALOAD, 1);
        mw.visitLdcInsn(DefaultJSONParser.NONE);
        mw.visitMethodInsn(INVOKEVIRTUAL, DefaultJSONParserStr, "setResolveStatus", "(I)V");
        mw.visitLabel(_end_if);
    }

    private void _deserObject(Context context,
                              MethodVisitor mw,
                              FieldInfo fieldInfo,
                              Class<?> fieldClass,
                              int i) {
        ...
        if ((fieldInfo.parserFeatures & Feature.SupportArrayToBean.mask) != 0) {
            mw.visitInsn(DUP);
            mw.visitTypeInsn(INSTANCEOF, ASMUtils.type(JavaBeanDeserializer.class));
            mw.visitJumpInsn(IFEQ, instanceOfElse_);
            mw.visitTypeInsn(CHECKCAST, ASMUtils.type(JavaBeanDeserializer.class)); // cast
            mw.visitVarInsn(ALOAD, 1);
            if (fieldInfo.fieldType instanceof Class) {
                mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(fieldInfo.fieldClass)));
            } else {
                mw.visitVarInsn(ALOAD, 0);
                mw.visitLdcInsn(i);
                mw.visitMethodInsn(INVOKEVIRTUAL,
    ...
    private void _getFieldDeser(Context context,
                                MethodVisitor mw,
                                FieldInfo fieldInfo) {
        Label notNull_ = new Label();
        mw.visitVarInsn(ALOAD, 0);
    ...
    private void _init(ClassWriter cw,
                       Context context) {
        for (int i = 0, size = context.fieldInfoList.length; i < size; ++i) {
            FieldInfo fieldInfo = context.fieldInfoList[i];
        ...
        MethodVisitor mw = new MethodWriter(cw,
                                            ...
                                            "<init>",
                                            "(" + ASMUtils.desc(ParserConfig.class)
                                                    + ASMUtils.desc(JavaBeanInfo.class) + ")V",
                                            null,
                                            null);
        mw.visitVarInsn(ALOAD, 0);
        mw.visitVarInsn(ALOAD, 1);
    ...
    private void _createInstance(ClassWriter cw,
                                 Context context) {
        ...
        mw.visitTypeInsn(NEW, ASMUtils.type(context.getInstClass()));
        mw.visitInsn(DUP);
        mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.type(context.getInstClass()), "<init>", "()V");
        mw.visitInsn(ARETURN);
        mw.visitMaxs(3, 3);
        mw.visitEnd();
    }
