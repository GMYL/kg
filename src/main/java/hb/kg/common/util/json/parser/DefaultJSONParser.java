...
import hb.kg.common.util.json.parser.deserializer.FieldTypeResolver;
...
import hb.kg.common.util.json.parser.deserializer.ResolveFieldDeserializer;
...
public class DefaultJSONParser implements Closeable {
    ...
    public DefaultJSONParser(final char[] input,
                             int length,
                             final ParserConfig config,
                             int features) {
    ...
    public DefaultJSONParser(final Object input,
        ...
        try {
            ...
            for (;;) {
                ...
                if (ch == '"') {
                    ...
                    final boolean parentIsArray = fieldName != null
                            && fieldName.getClass() == Integer.class;
                    // if (!parentIsArray) {
                ...
                if (ch == ',') {
                    ...
                    throw new JSONException("syntax error, position at " + lexer.pos() + ", name "
                            + key);
                }
            }
        } finally {
    ...
    public void parseArray(Type type,
        ...
        if (token != JSONToken.LBRACKET) {
            throw new JSONException("exepct '[', but " + JSONToken.name(token) + ", "
                    + lexer.info());
        }
        ...
        for (;;) {
            ...
            if (fieldDeser == null) {
                if (!lexer.isEnabled(Feature.IgnoreNotMatch)) {
                    throw new JSONException("setter not found, class " + clazz.getName()
                            + ", property " + key);
                }
                ...
                if (fieldClass == int.class) {
                    ...
                    ObjectDeserializer fieldValueDeserializer = config.getDeserializer(fieldClass,
                                                                                       fieldType);
    ...
    public void checkMapResolve(Map object,
                                Object fieldName) {
        if (resolveStatus == NeedToResolve) {
            ResolveFieldDeserializer fieldResolver = new ResolveFieldDeserializer(object,
                                                                                  fieldName);
            ResolveTask task = getLastResolveTask();
    ...
    public final void parseArray(final Collection array,
        ...
        if (lexer.token() != JSONToken.LBRACKET) {
            throw new JSONException("syntax error, expect [, actual "
                    + JSONToken.name(lexer.token()) + ", pos " + lexer.pos() + ", fieldName "
                    + fieldName);
        }
        lexer.nextToken(JSONToken.LITERAL_STRING);
        ParseContext context = this.context;
    ...
    public void handleResovleTask(Object value) {
        ...
        for (int i = 0, size = resolveTaskList.size(); i < size; ++i) {
            ...
            if (fieldDeser != null) {
                if (refValue != null && refValue.getClass() == JSONObject.class
    ...
    public static class ResolveTask {
        ...
        public ResolveTask(ParseContext context,
    ...
    public void parseExtra(Object object,
                           String key) {
    ...
    @SuppressWarnings("unchecked")
    public Object parse(PropertyProcessable object,
                        Object fieldName) {
        ...
        try {
            for (int i = 0;; ++i) {
                ...
                if (key == JSON.DEFAULT_TYPE_KEY
                    ...
                    return (Map<Object, Object>) deserializer.deserialze(this, clazz, fieldName);
                }
                Object value;
                lexer.nextToken();
                if (i != 0) {
