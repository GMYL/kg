...
public class ASMSerializerFactory implements Opcodes {
    ...
    public JavaBeanSerializer createJavaBeanSerializer(SerializeBeanInfo beanInfo)
        ...
        for (FieldInfo fieldInfo : getters) {
            ...
            if (List.class.isAssignableFrom(fieldInfo.fieldClass)) {
                new FieldWriter(cw,
                                ACC_PUBLIC,
                                fieldInfo.name + "_asm_list_item_ser_",
        ...
        MethodVisitor mw = new MethodWriter(cw,
                                            ACC_PUBLIC,
                                            "<init>",
                                            "(" + ASMUtils.desc(SerializeBeanInfo.class) + ")V",
                                            null,
                                            null);
        ...
        for (int i = 0; i < getters.length; ++i) {
            ...
            if (fieldInfo.method != null) {
                mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(fieldInfo.declaringClass)));
                mw.visitLdcInsn(fieldInfo.method.getName());
                mw.visitMethodInsn(INVOKESTATIC,
                                   ASMUtils.type(ASMUtils.class),
                                   "getMethodType",
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            mw.visitTypeInsn(CHECKCAST, ASMUtils.type(clazz)); // serializer
        ...
        Class<?> serializerClass = classLoader.defineClassPublic(classNameFull,
                                                                 code,
                                                                 0,
                                                                 code.length);
    ...
    private void generateWriteAsArray(Class<?> clazz,
                                      MethodVisitor mw,
                                      FieldInfo[] getters,
                                      Context context)
            throws Exception {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (fieldClass == byte.class //
                ...
                if (elementClass == String.class //
                    ...
                    mw.visitJumpInsn(IF_ICMkgE, forEnd_); // i < list.size - 1
                    mw.visitVarInsn(ILOAD, context.var("i"));
                    ...
                    if (elementClass != null && Modifier.isPublic(elementClass.getModifiers())) {
                        ...
                        mw.visitMethodInsn(INVOKEVIRTUAL,
                                           ...
                                           "getClass",
                                           "()Ljava/lang/Class;");
                        mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(elementClass)));
                        ...
                        if (context.writeDirect) {
                            ...
                            mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(elementClass))); // fieldType
                    ...
                    if (elementClass != null && Modifier.isPublic(elementClass.getModifiers())) {
                        mw.visitLdcInsn(ASMType.getType(ASMUtils.desc((Class<?>) elementType)));
                ...
                if (context.writeDirect && Modifier.isPublic(fieldClass.getModifiers())) {
                    ...
                    mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(fieldClass))); // fieldType
                    mw.visitLdcInsn(fieldInfo.serialzeFeatures); // features
                    mw.visitMethodInsn(INVOKEVIRTUAL,
                                       JavaBeanSerializer,
                                       "writeAsArrayNonContext", //
    ...
    private void generateWriteMethod(Class<?> clazz,
                                     MethodVisitor mw,
                                     FieldInfo[] getters,
                                     Context context)
            throws Exception {
        ...
        boolean writeClasName = (context.beanInfo.features
                & SerializerFeature.WriteClassName.mask) != 0;
        // SEPERATO
        if (writeClasName || !context.writeDirect) {
            ...
            mw.visitMethodInsn(INVOKEVIRTUAL,
                               JavaBeanSerializer,
                               "writeClassName",
                               "(L" + JSONSerializer + ";Ljava/lang/String;Ljava/lang/Object;)V");
            mw.visitVarInsn(BIPUSH, ',');
    ...
    private void _object(Class<?> clazz,
                         MethodVisitor mw,
                         FieldInfo property,
                         Context context) {
    ...
    private void _enum(Class<?> clazz,
                       MethodVisitor mw,
                       FieldInfo fieldInfo,
                       Context context) {
        ...
        if (context.writeDirect) {
            ...
            mw.visitMethodInsn(INVOKEVIRTUAL,
                               SerializeWriter,
                               "writeFieldValueStringWithDoubleQuote",
            ...
            mw.visitVarInsn(ALOAD, context.var("enum"));
            mw.visitVarInsn(ALOAD, Context.fieldName);
            mw.visitLdcInsn(ASMType.getType(ASMUtils.desc((Class<?>) fieldInfo.fieldClass)));
            mw.visitLdcInsn(fieldInfo.serialzeFeatures);
            mw.visitMethodInsn(INVOKEVIRTUAL,
                               JSONSerializer,
                               "writeWithFieldName",
    ...
    private void _int(Class<?> clazz,
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           SerializeWriter,
                           "writeFieldValue",
                           "(CLjava/lang/String;" + type + ")V");
        _seperator(mw, context);
        mw.visitLabel(end_);
    }

    private void _long(Class<?> clazz,
                       MethodVisitor mw,
                       FieldInfo property,
                       Context context) {
        Label end_ = new Label();
    ...
    private void _float(Class<?> clazz,
                        MethodVisitor mw,
                        FieldInfo property,
                        Context context) {
        Label end_ = new Label();
    ...
    private void _double(Class<?> clazz,
                         MethodVisitor mw,
                         FieldInfo property,
                         Context context) {
        Label end_ = new Label();
    ...
    private void _get(MethodVisitor mw,
                      Context context,
                      FieldInfo fieldInfo) {
        ...
        if (method != null) {
            ...
            mw.visitMethodInsn(declaringClass.isInterface() ? INVOKEINTERFACE : INVOKEVIRTUAL,
                               ASMUtils.type(declaringClass),
                               method.getName(),
                               ASMUtils.desc(method));
            if (!method.getReturnType().equals(fieldInfo.fieldClass)) {
                mw.visitTypeInsn(CHECKCAST, ASMUtils.type(fieldInfo.fieldClass)); // cast
            }
        } else {
            mw.visitVarInsn(ALOAD, context.var("entity"));
    ...
    private void _decimal(Class<?> clazz,
                          MethodVisitor mw,
                          FieldInfo property,
                          Context context) {
        Label end_ = new Label();
    ...
    private void _string(Class<?> clazz,
                         MethodVisitor mw,
                         FieldInfo property,
                         Context context) {
        Label end_ = new Label();
    ...
    private void _list(Class<?> clazz,
                       MethodVisitor mw,
                       FieldInfo fieldInfo,
                       Context context) {
    ...
    private void _filters(MethodVisitor mw,
                          FieldInfo property,
                          Context context,
                          Label _end) {
    ...
    private void _nameApply(MethodVisitor mw,
                            FieldInfo property,
                            Context context,
                            Label _end) {
        if (!context.writeDirect) {
    ...
    private void _labelApply(MethodVisitor mw,
                             FieldInfo property,
                             Context context,
                             Label _end) {
        mw.visitVarInsn(ALOAD, 0); // this
    ...
    private void _writeObject(MethodVisitor mw,
                              FieldInfo fieldInfo,
                              Context context,
                              Label _end) {
        String format = fieldInfo.getFormat();
        ...
        if (Modifier.isPublic(fieldClass.getModifiers()) //
            ...
            mw.visitTypeInsn(INSTANCEOF, JavaBeanSerializer);
            mw.visitJumpInsn(IFEQ, instanceOfElse_);
            boolean disableCircularReferenceDetect = (fieldInfo.serialzeFeatures
                    & SerializerFeature.DisableCircularReferenceDetect.mask) != 0;
            boolean fieldBeanToArray = (fieldInfo.serialzeFeatures
                    & SerializerFeature.BeanToArray.mask) != 0;
            String writeMethodName;
            if (disableCircularReferenceDetect || (context.nonContext && context.writeDirect)) {
                writeMethodName = fieldBeanToArray ? "writeAsArrayNonContext"
                        : "writeDirectNonContext";
            } else {
        ...
        if (format != null) {
            ...
            if (fieldInfo.fieldType instanceof Class<?> //
                ...
                if (fieldInfo.fieldClass == String.class) {
                    mw.visitLdcInsn(ASMType.getType(ASMUtils.desc(String.class)));
                } else {
                    mw.visitVarInsn(ALOAD, 0);
                    mw.visitFieldInsn(GETFIELD,
                                      context.className,
    ...
    private void _before(MethodVisitor mw,
                         Context context) {
        mw.visitVarInsn(ALOAD, 0); // this
        mw.visitVarInsn(ALOAD, Context.serializer);
        mw.visitVarInsn(ALOAD, Context.obj);
    ...
    private void _after(MethodVisitor mw,
                        Context context) {
        mw.visitVarInsn(ALOAD, 0); // this
        mw.visitVarInsn(ALOAD, Context.serializer);
        mw.visitVarInsn(ALOAD, 2); // obj
    ...
    private void _notWriteDefault(MethodVisitor mw,
                                  FieldInfo property,
                                  Context context,
                                  Label _end) {
        if (context.writeDirect) {
    ...
    private void _apply(MethodVisitor mw,
                        FieldInfo property,
                        Context context) {
        Class<?> propertyClass = property.fieldClass;
        mw.visitVarInsn(ALOAD, 0); // this
    ...
    private void _processValue(MethodVisitor mw,
                               FieldInfo fieldInfo,
                               Context context,
                               Label _end) {
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           JavaBeanSerializer,
                           "getBeanContext",
                           "(I)" + ASMUtils.desc(BeanContext.class));
        mw.visitVarInsn(ALOAD, Context.obj);
        mw.visitVarInsn(ALOAD, Context.fieldName);
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL,
                           ...
                           "processValue",
                           "(L" + JSONSerializer + ";" //
                                   + ASMUtils.desc(BeanContext.class) //
    ...
    private void _processKey(MethodVisitor mw,
                             FieldInfo property,
                             Context context) {
    ...
    private void _if_write_null(MethodVisitor mw,
                                FieldInfo fieldInfo,
                                Context context) {
    ...
    private void _writeFieldName(MethodVisitor mw,
                                 Context context) {
        if (context.writeDirect) {
            mw.visitVarInsn(ALOAD, context.var("out"));
            mw.visitVarInsn(ALOAD, Context.fieldName);
    ...
    private void _seperator(MethodVisitor mw,
                            Context context) {
        mw.visitVarInsn(BIPUSH, ',');
        mw.visitVarInsn(ISTORE, context.var("seperator"));
    }

    private void _getListFieldItemSer(Context context,
                                      MethodVisitor mw,
                                      FieldInfo fieldInfo,
                                      Class<?> itemType) {
        Label notNull_ = new Label();
    ...
    private void _getFieldSer(Context context,
                              MethodVisitor mw,
                              FieldInfo fieldInfo) {
        Label notNull_ = new Label();
        mw.visitVarInsn(ALOAD, 0);
