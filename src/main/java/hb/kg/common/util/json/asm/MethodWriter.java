...
public class MethodWriter implements MethodVisitor {
    ...
    int exceptionCount;
    /**
     * The exceptions that can be thrown by this method. More precisely, this array
     * contains the indexes of the constant pool items that contain the internal
     * names of these exception classes.
     */
    int[] exceptions;
    /**
     ...
     * "basic block", and one edge per "jump" from one basic block to another. Each
    ...
    // Constructor
    // ------------------------------------------------------------------------
    public MethodWriter(final ClassWriter cw,
                        final int access,
                        final String name,
                        final String desc,
    ...
    public void visitMethodInsn(final int opcode,
        ...
        if (itf) {
            if (argSize == 0) {
                argSize = ASMType.getArgumentsAndReturnSizes(desc);
                i.intVal = argSize;
            }
    ...
    public void visitJumpInsn(final int opcode,
        ...
        if ((label.status & 2 /* Label.RESOLVED */ ) != 0
            ...
            /*
             * case of a backward jump with an offset >= -32768, or of a forward jump with,
             * of course, an unknown offset. In these cases we store the offset in 2 bytes
             * (which will be increased in resizeInstructions, if needed).
             */
            code.putByte(opcode);
