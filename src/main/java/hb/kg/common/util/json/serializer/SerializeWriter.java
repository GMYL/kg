...
import hb.kg.common.util.reflect.RyuDouble;
import hb.kg.common.util.reflect.RyuFloat;
...
public final class SerializeWriter extends Writer {
    ...
    final static int nonDirectFeatures = 0 //
            ...
            | SerializerFeature.WriteSlashAsSpecial.mask | SerializerFeature.IgnoreErrorGetter.mask
            | SerializerFeature.WriteClassName.mask | SerializerFeature.NotWriteDefaultValue.mask;
    ...
    protected void computeFeatures() {
        ...
        writeEnumUsingToString = (this.features
                & SerializerFeature.WriteEnumUsingToString.mask) != 0;
    ...
    public void expandCapacity(int minimumCapacity) {
        if (maxBufSize != -1 && minimumCapacity >= maxBufSize) {
            throw new JSONException("serialize exceeded MAX_OUTPUT_LENGTH=" + maxBufSize
    ...
    public int writeToEx(OutputStream out,
                         Charset charset)
            throws IOException {
        if (this.writer != null) {
            throw new UnsupportedOperationException("writer not null");
        ...
        if (newcount > buf.length) {
            if (writer != null) {
                ...
                for (int s = 0; s < eLen;) {
                    ...
                    int i = (bytes[s++] & 0xff) << 16 | (bytes[s++] & 0xff) << 8
                            | (bytes[s++] & 0xff);
                ...
                if (left > 0) {
                    // Prepare the int
                    int i = ((bytes[eLen] & 0xff) << 10)
                            | (left == 2 ? ((bytes[bytesLen - 1] & 0xff) << 2) : 0);
                    // Set last four chars
                    write(CA[i >> 12]);
                    write(CA[(i >>> 6) & 0x3f]);
    ...
    public void writeNull(int beanFeatures,
                          int feature) {
    ...
    public void writeStringWithDoubleQuote(String text,
                ...
                for (int i = 0; i < text.length(); ++i) {
                    ...
                    if (isEnabled(SerializerFeature.BrowserCompatible)) {
                        ...
                        if (ch < IOUtils.specicalFlags_doubleQuotes.length
                                ...
                                || (ch == '/'
                                        && isEnabled(SerializerFeature.WriteSlashAsSpecial))) {
                            write('\\');
        ...
        if (specialCount > 0) {
            ...
            if (specialCount == 1) {
                ...
                } else if (lastSpecial == '(' || lastSpecial == ')' || lastSpecial == '<'
                        || lastSpecial == '>') {
                    int srcPos = lastSpecialIndex + 1;
                    int destPos = lastSpecialIndex + 6;
                    int LengthOfCopy = end - lastSpecialIndex - 1;
    ...
    public void writeStringWithDoubleQuote(char[] text,
                                           final char seperator) {
        if (text == null) {
            writeNull();
            if (seperator != 0) {
    ...
    public void writeFieldValueStringWithDoubleQuoteCheck(char seperator,
                                                          String name,
                                                          String value) {
        int nameLen = name.length();
        int valueLen;
    ...
    public void writeFieldValueStringWithDoubleQuote(char seperator,
                                                     String name,
                                                     String value) {
        int nameLen = name.length();
        int valueLen;
    ...
    private void writeEnumFieldValue(char seperator,
                                     String name,
                                     String value) {
        if (useSingleQuotes) {
    ...
    public void writeFieldValue(char seperator,
        ...
        if (value == null) {
            ...
            write(isEnabled(SerializerFeature.WriteBigDecimalAsPlain) && scale >= -100
                    && scale < 100 ? value.toPlainString() : value.toString());
        }
    }
            ...
            if (writer != null) {
                ...
                for (int i = 0; i < len; ++i) {
                    ...
                    if (ch < specicalFlags_singleQuotes.length
                            && specicalFlags_singleQuotes[ch] != 0) {
                        hasSpecial = true;
                        break;
                    }
                }
