...
public abstract class JSONLexerBase implements JSONLexer, Closeable {
    ...
    // public abstract String scanSymbol(final SymbolTable symbolTable, final char
    // quote);
    protected abstract void arrayCopy(int srcPos,
                                      char[] dest,
                                      int destPos,
                                      int length);
    ...
    public final String scanSymbolUnQuoted(final SymbolTable symbolTable) {
        ...
        if (sp == 4 && hash == NULL_HASH && charAt(np) == 'n' && charAt(np + 1) == 'u'
                && charAt(np + 2) == 'l' && charAt(np + 3) == 'l') {
            return null;
        }
    ...
    protected abstract void copyTo(int offset,
                                   int count,
                                   char[] dest);
    ...
    protected final static char[] typeFieldName = ("\"" + JSON.DEFAULT_TYPE_KEY
            + "\":\"").toCharArray();
    ...
    public final boolean matchField(char[] fieldName) {
        ...
        if (ch == '{') {
            ...
            next();
            token = JSONToken.LBRACKET;
        } else if (ch == 'S' && charAt(bp + 1) == 'e' && charAt(bp + 2) == 't'
                && charAt(bp + 3) == '[') {
            bp += 3;
            ch = charAt(bp);
            token = JSONToken.SET;
    ...
    public String scanString(char expectNextChar) {
        ...
        char chLocal = charAt(bp + (offset++));
        if (chLocal == 'n') {
            if (charAt(bp + offset) == 'u' && charAt(bp + offset + 1) == 'l'
    ...
    public String scanSymbolWithSeperator(final SymbolTable symbolTable,
                                          char serperator) {
        matchStat = UNKNOWN;
        int offset = 0;
        char chLocal = charAt(bp + (offset++));
    ...
    public void scanStringArray(Collection<String> list,
                                char seperator) {
        matchStat = UNKNOWN;
        int offset = 0;
        char chLocal = charAt(bp + (offset++));
        if (chLocal == 'n' && charAt(bp + offset) == 'u' && charAt(bp + offset + 1) == 'l'
                && charAt(bp + offset + 2) == 'l' && charAt(bp + offset + 3) == seperator
        ) {
            bp += 5;
            ch = charAt(bp);
    ...
    public int scanInt(char expectNext) {
        ...
        } else if (chLocal == 'n' && charAt(bp + offset) == 'u' && charAt(bp + offset + 1) == 'l'
                && charAt(bp + offset + 2) == 'l') {
            matchStat = VALUE_NULL;
            value = 0;
            offset += 3;
    ...
    public java.util.UUID scanFieldUUID(char[] fieldName) {
        ...
        } else if (chLocal == 'n' && charAt(bp + (offset++)) == 'u'
                && charAt(bp + (offset++)) == 'l' && charAt(bp + (offset++)) == 'l') {
            uuid = null;
            chLocal = charAt(bp + (offset++));
        } else {
            matchStat = NOT_MATCH;
        ...
        for (;;) {
            ...
            if (chLocal == '\\') {
                ...
                case 'x':
                    ...
                    break;
                case 'u':
                    putChar((char) Integer.parseInt(new String(new char[] { next(),
                                                                            next(),
                                                                            next(),
                                                                            next() }),
                                                    16));
     ...
     * @return
     */
    public String[] scanFieldStringArray(char[] fieldName,
                                         int argTypesCount,
