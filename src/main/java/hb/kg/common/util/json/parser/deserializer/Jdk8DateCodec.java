...
public class Jdk8DateCodec extends ContextObjectDeserializer
    ...
    private final static DateTimeFormatter ISO_FIXED_FORMAT = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
        ...
        if (lexer.token() == JSONToken.LITERAL_STRING) {
            ...
            if (type == LocalDateTime.class) {
                ...
                if (text.length() == 23) {
                    ...
                    localDate = LocalDate.of(localDateTime.getYear(),
                ...
                if (text.length() == 23) {
                    LocalDateTime localDateTime = LocalDateTime.parse(text);
                    localDate = LocalTime.of(localDateTime.getHour(),
            ...
            lexer.nextToken();
            if (type == LocalDateTime.class) {
                return (T) LocalDateTime.ofInstant(Instant.ofEpochMilli(millis),
    ...
    protected LocalDate parseLocalDate(String text,
                                       String format,
                                       DateTimeFormatter formatter) {
        if (formatter == null) {
            if (text.length() == 8) {
    ...
    protected ZonedDateTime parseZonedDateTime(String text,
                                               DateTimeFormatter formatter) {
        if (formatter == null) {
            if (text.length() == 19) {
                char c4 = text.charAt(4);
        ...
        if (object == null) {
            ...
            if (fieldType == LocalDateTime.class) {
                ...
                if (format != null) {
                    ...
                    out.writeLong(dateTime.atZone(JSON.defaultTimeZone.toZoneId())
                                          .toInstant()
                                          .toEpochMilli());
                }
            } else {
    ...
    private void write(SerializeWriter out,
