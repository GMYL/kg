...
public abstract class AbstractDateDeserializer extends ContextObjectDeserializer
        ...
        if (lexer.token() == JSONToken.LITERAL_INT) {
                ...
                if (val == null && JSON.defaultLocale == Locale.CHINA) {
                    try {
                        ...
                        if (format.contains("T")) {
                            String fromat2 = format.replaceAll("T", "'T'");
                            try {
                                simpleDateFormat = new SimpleDateFormat(fromat2,
                                                                        JSON.defaultLocale);
                            } catch (IllegalArgumentException e2) {
                                throw ex;
                            }
            ...
            if (lexer.token() == JSONToken.LITERAL_STRING) {
                ...
                if (JSON.DEFAULT_TYPE_KEY.equals(key)) {
                    ...
                    Class<?> type = parser.getConfig()
                                          .checkAutoType(typeName, null, lexer.getFeatures());
                    if (type != null) {
                        clazz = type;
                    }
    ...
    protected abstract <T> T cast(DefaultJSONParser parser,
                                  Type clazz,
                                  Object fieldName,
                                  Object value);
}
