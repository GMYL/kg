...
public class IDUtil {
    ...
    public static String generateTimedMinuteStr() {
        return TimerService.now_to_min;
    }
    ...
    public static String generateASpliceId() {
        return TimerService.now_to_min + "-" + generateRandomKey(7);
    }

    /**
     * 生成订单id号，输入参数是用户id号 格式是用户id号后四位+时间标志
     */
    // public static String generateOrderIdStr(String cusid) {
    // StringBuffer sb = new StringBuffer();
    // if (cusid.length() < 4) {
    // sb.append(cusid);
    // } else {
    // sb.append(cusid.substring(cusid.length() - 4));
    // }
    ...
    /**
     * 生成账单id号 （28位） 生成方法：p_ + 8位日期 + 6位时间 + 6位随机数 + 6位纳秒时间 例：p_ 20181019 165143
     * 520365 639660
     */
    public static String generatePaymentIdStr() {
        // 生成时间 年月日时分秒
        Date dt = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
        String timeString = sdf.format(dt);
        // 取后8位纳秒值
        Long nano = System.nanoTime() % 100000000;
        // 6位随机数
        Integer random = new Random().nextInt(1000000);
        return timeString + String.format("%06d", random) + String.format("%08d", nano);
    }

    /**
     * 生成钱包id号 （18位） 生成方法：w_ + 14位时间信息 + 2位随机字符 例：12479083317097 asda
     */
    public static String generateWalletIdStr() {
        return "w_" + generateTimedID() + generateRandomKey(2);
    }
