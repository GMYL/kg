...
import hb.kg.common.dao.BaseRedisCacheDao;
import hb.kg.common.util.encrypt.AESUtil;
 ...
 */
@Service
public class AuthService extends BaseRedisCacheDao<HBUserSession> {
    @Resource
    private UserDao userDao;
    @Autowired
    private ModuleService moduleService;
    public Collection<String> secondaryIndex = HBCollectionUtil.stringToList("name,phoneNum", ",");
    @Autowired
    protected HttpServletRequest request;

    @Override
    public Class<HBUserSession> getClassT() {
        return HBUserSession.class;
    }
    ...
    public HBUserSession generateToken(HBUserSession session) {
        ...
        { // 对秘钥的后半部分加密
            ...
            // 增加phonenum
            sb.append(session.getUser().getPhoneNum()).append(encryptTok);
            // 生成token的初始值
            String rawToken = sb.toString();
            try {
                // token加密
    ...
    public HBUserSession login(String username,
        ...
        HBUserSession session = findOne(username, "name");
        if (session == null) {
            // 如果session不存在，新建并判断密码是否一致
            HBUser user = userDao.findUserByName(username);
            if (user != null) {
                session = new HBUserSession(user);
                session.setLastVisitUrl(request.getRemoteAddr());//GOO
     ...
     * 按照用户名直接登录
     */
    public HBUserSession loginByName(String username) {
        // 先去session中查询有没有session，如果有，直接拿出来比对即可
        HBUserSession session = findOne(username, "name");
        if (session == null) {
            HBUser user = userDao.findUserByName(username);
    ...
    public HBUserSession loginById(String userid) {
        // 先去session中查询有没有session，如果有，直接拿出来比对即可
        HBUserSession session = findOne(userid);
        if (session == null) {
    ...
    public HBUserSession loginByPhone(String phoneNum) {
        // 先去session中查询有没有session，如果有，直接拿出来比对即可
        HBUserSession session = findOne(phoneNum, "phoneNum");
        if (session == null) {
            HBUser user = userDao.findUserByPhoneNum(phoneNum);
            if (user != null) {
                session = new HBUserSession(user);
                insert(session);
            }
    ...
    public HBUserSession checkSessionValid(String sessionKey) {
        HBUserSession session = findOne(sessionKey);
        if (session != null) {
    ...
    public HBUserSession checkTokenValid(String encryToken) {
        try {
            String sessionkey = encryToken.split("\\.")[0];
            // 先查询当前有没有用户的session，如果session有，通过session检查
            HBUserSession session = null;
            if ((session = checkSessionValid(sessionkey)) != null) {
                return session;
            }
    ...
    public HBUserSession getUserSessionByToken(String encryToken) {
        try {
            ...
            // 先查询当前有没有用户的session，如果session有，通过session检查
            HBUserSession session = null;
            if ((session = checkSessionValid(sessionkey[0])) != null) {
                return session;
            }
